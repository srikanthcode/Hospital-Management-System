...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $email && $password) {
        ...
        if (mysqli_fetch_assoc(mysqli_stmt_get_result($stmt))) {
            ...
            $stmt = mysqli_prepare($conn, "INSERT INTO users (name,email,password,role) VALUES (?,?,?, 'nurse')");
            mysqli_stmt_bind_param($stmt, "sss", $name, $email, $hash);
            if (mysqli_stmt_execute($stmt)) {
                $uid = mysqli_insert_id($conn);
                $stmt = mysqli_prepare($conn, "INSERT INTO nurses (user_id,name,phone,email,shift,department,address) VALUES (?,?,?,?,?,?,?)");
                mysqli_stmt_bind_param($stmt, "issssss", $uid, $name, $phone, $email, $shift, $department, $address);
                mysqli_stmt_execute($stmt);
                $message = "Nurse registered successfully. Please login.";
            } else {
...
<head>
    ...
    <title>Nurse Registration - Lotus Women's Hospital</title>
...
<div class="container py-5">
  <div class="row justify-content-center">
    <div class="col-md-7">
      <div class="card p-4">
        <h3 class="pink-heading">Nurse Registration</h3>
        <?php if ($message): ?>
        ...
        <form method="post">
          <div class="row g-2">
            ...
            <div class="col-md-6 mb-2"><label class="form-label">Department</label>
              <input class="form-control" name="department" placeholder="e.g. Maternity">
            </div>
